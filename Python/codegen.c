    ...
    if (gen_index >= asdl_seq_LEN(generators)) {
        ...
        case COMP_DICTCOMP:
            ...
            if (elt == NULL) {
                ...
                /* “{ **expr  for … }”  →  DICT_MERGE */
                //VISIT(c, expr, val);     /* mapping expression        */
                //elt_loc = LOCATION(val->lineno,
                //                   val->end_lineno,
                //                   val->col_offset,
                //                   val->end_col_offset);
                /* Merge into the comprehension result (which lives
                   'depth + 1' slots below TOS, same convention as
                   MAP_ADD for k:v pairs).                            */
                //ADDOP_I(c, elt_loc, DICT_MERGE, depth + 1);
